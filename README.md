...
現在は、特定の TouchDesigner 構成の再現だけでなく、汎用的な typed graph tool として扱える方向へ寄せています。
...
template ボタンからは generic node や signal/image node を追加できます。
...
このプロジェクトでは、上の構成を typed graph document と node registry で意味的に再現しています。
